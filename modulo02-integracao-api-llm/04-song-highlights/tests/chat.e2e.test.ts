...
describe('Chat de Recomendação Musical - Testes E2E', () => {
  ...
  it('Deve criar um resumo da conversa', async () => {
    const userId = 'test-taylor';
    const threadId = `${userId}-${Date.now()}`;
    const config = {
      configurable: { thread_id: threadId },
    ...
    const response = await graph.invoke(
      {
        messages: [
          new HumanMessage('Oi, sou Henrique e gosto de musica de rock'),
          new HumanMessage('Tenho 28 anos'),
          new HumanMessage('Gosto Nu Metal e Rock Alternativo'),
          new HumanMessage('SOAD e Mudvayne, gosto muito !'),
          new HumanMessage('Fui no Iron Maiden em 2016, foi incrível!'),
          new HumanMessage('Perdi o show do SOAD em 2025, mas em 2026 vou no Iron Maiden de novo'),
        ],
        userId,
      },
      config
